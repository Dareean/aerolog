<x-app-layout :noNav="true">
    <div class="min-h-screen flex items-center justify-center bg-[#fcf9f8] py-12 px-4 sm:px-6 lg:px-8">
        <div class="w-full max-w-[440px] bg-canvas border border-surface-strong rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-8 sm:p-10" data-reveal>
            <div class="mb-6">
            ...
            <div class="text-center mb-8">
                <div class="mb-3 text-[12px] font-semibold uppercase tracking-[0.18em] text-[#0d74ce]">
                    <span class="type-cursor" data-typewriter='@json([__("Masuk cepat ke AeroLog."), __("Lanjut ke flight deck."), __("Kelola log tanpa hambatan.")])'>{{ __('Masuk cepat ke AeroLog.') }}</span>
                </div>
                <h1 class="font-display-md text-[28px] font-bold text-primary mb-2 tracking-tight">{{ __('Sign in to AeroLog') }}</h1>
                <p class="font-body-md text-[15px] text-on-surface-variant">{{ __('Enter your credentials to access the flight deck.') }}</p>
            </div>
                ...
                <div class="mb-6 text-left">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-[13px] font-medium text-[#0d74ce] hover:underline">{{ __('Forgot password?') }}</a>
                        @endif
                    </div>
            ...
            <div class="mt-8 border-t border-surface-strong pt-6 text-center">
                <p class="text-[14px] text-on-surface-variant">
                    {{ __('Don\'t have an account?') }} <a href="#" class="font-medium text-[#0d74ce] hover:underline">{{ __('Contact your dispatcher') }}</a>
                </p>
            </div>
        </div>
    </div>
