        ...
        <div class="w-full max-w-[480px] bg-canvas border border-surface-strong rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-8 sm:p-10" data-reveal>
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5 text-left">
                    <div>
                        <label for="username" class="block font-title-md text-[14px] font-semibold text-on-surface mb-2">{{ __('Username') }}</label>
                    ...
                    <div>
                        <label for="full_name" class="block font-title-md text-[14px] font-semibold text-on-surface mb-2">{{ __('Full Name') }}</label>
                ...
                <div class="mb-5 text-left">
                    <label for="email" class="block font-title-md text-[14px] font-semibold text-on-surface mb-2">{{ __('Email address') }}</label>
                ...
                <!-- Role -->
                <div class="mb-5 text-left">
                    <label for="role" class="block font-title-md text-[14px] font-semibold text-on-surface mb-2">{{ __('Role') }}</label>
                    <select id="role" name="role" required class="w-full h-[44px] px-3 border border-[#e5e2e1] rounded-lg focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none font-body-md text-primary bg-white shadow-sm transition-colors">
                        ...
                        <option value="pilot" {{ old('role') == 'pilot' ? 'selected' : '' }}>{{ __('Pilot') }}</option>
                        <option value="dispatcher" {{ old('role') == 'dispatcher' ? 'selected' : '' }}>{{ __('Dispatcher') }}</option>
                    </select>
                ...
                <div class="grid grid-cols-2 gap-4 mb-6 text-left">
                    <!-- Password -->
                    <div>
                        <label for="password" class="block font-title-md text-[14px] font-semibold text-on-surface mb-2">{{ __('Password') }}</label>
                    ...
                    <!-- Confirm Password -->
                    <div>
                        <label for="password_confirmation" class="block font-title-md text-[14px] font-semibold text-on-surface mb-2">{{ __('Confirm Password') }}</label>
            ...
            <div class="mt-8 border-t border-surface-strong pt-6 text-center">
                <p class="text-[14px] text-on-surface-variant">
                    {{ __('Already registered?') }} <a href="{{ route('login') }}" class="font-medium text-[#0d74ce] hover:underline">{{ __('Sign in to AeroLog') }}</a>
                </p>
            </div>
        </div>
    </div>
